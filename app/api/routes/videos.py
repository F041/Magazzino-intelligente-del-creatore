...
def _process_youtube_channel_core(channel_id: str, user_id: Optional[str], core_config: dict) -> bool:
    ...
    Recupera video da YT, confronta con DB (filtrando per user_id se SAAS),
    processa i nuovi (trascrizione, embedding, Chroma), salva nel DB.
    NON gestisce lo stato globale del thread UI.
    ...
    overall_success = False # Flag per il successo generale
    conn_sqlite = None
    ...
    try:
        ...
        try:
            # Ora puoi chiamare il metodo perché youtube_client esiste
        ...
        if app_mode == 'saas':
            ...
            log_msg_filter = f" e utente '{user_id}'"
        logger.info(f"[CORE YT Process] Controllo video esistenti per canale '{channel_id}'{log_msg_filter}.")
        cursor_sqlite.execute(sql_check_existing, tuple(params_check_existing))
        rows = cursor_sqlite.fetchall(); existing_video_ids = {row[0] for row in rows}
        db_existing_count = len(existing_video_ids)
        logger.info(f"[CORE YT Process] Trovati {db_existing_count} video esistenti nel DB per questo utente/canale.")
        ...
        if to_process_count == 0:
            logger.info("[CORE YT Process] Nessun nuovo video da processare.")
            overall_success = True # Consideriamo successo se non c'era nulla da fare
        else:
            # --- OTTIENI COLLEZIONE CHROMA CORRETTA ---
            chroma_collection_to_use = None; collection_name_for_log = "N/A"
            if app_mode == 'single':
                ...
                if chroma_collection_to_use: collection_name_for_log = chroma_collection_to_use.name
                 ...
                 # Assumiamo user_id e chroma_client validati prima
                user_video_collection_name = f"{base_video_collection_name}_{user_id}"
                collection_name_for_log = user_video_collection_name
                try: chroma_collection_to_use = chroma_client.get_or_create_collection(name=user_video_collection_name); logger.info(f"[CORE YT Process] Collezione SAAS '{collection_name_for_log}' pronta.")
                except Exception as e_saas_coll: logger.error(f"[CORE YT Process] Fallimento get/create collezione SAAS '{collection_name_for_log}': {e_saas_coll}"); chroma_collection_to_use = None # Segna come non disponibile

            if not chroma_collection_to_use:
                 logger.warning(f"[CORE YT Process] Collezione Chroma '{collection_name_for_log}' non disponibile, l'upsert sarà saltato.")
                 # Non impostiamo errore critico qui, ma l'upsert fallirà o verrà saltato

            # --- 5. CICLO ELABORAZIONE VIDEO NUOVI ---
            logger.info(f"[CORE YT Process] Inizio ciclo elaborazione {to_process_count} video...")
            for index, video_model in enumerate(videos_to_process_models, 1):
                video_id = video_model.video_id
                video_title = video_model.title
                logger.info(f"[CORE YT Process] --- Processing NEW video {index}/{to_process_count}: {video_id} ({video_title}) ---")
                ...
                try:
                    # 5.a Trascrizione
                    logger.debug(f"[CORE YT Process] [{video_id}] Recupero trascrizione...")
                    ...
                    if transcript_result:
                         ...
                         current_video_status = 'processing_embedding'; logger.info(f"[CORE YT Process] [{video_id}] Trascrizione OK.")
                    else: current_video_status = 'failed_transcript'; transcript_errors += 1; logger.warning(f"[CORE YT Process] [{video_id}] Trascrizione fallita.")

                    # 5.b Chunking
                    if current_video_status == 'processing_embedding' and transcript_text:
                         logger.debug(f"[CORE YT Process] [{video_id}] Chunking...")
                         chunks = split_text_into_chunks(transcript_text, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
                         if not chunks: current_video_status = 'completed'; logger.info(f"[CORE YT Process] [{video_id}] No chunks.")
                         else: logger.info(f"[CORE YT Process] [{video_id}] {len(chunks)} chunks.")
                    elif current_video_status != 'failed_transcript':
                         current_video_status = 'failed_transcript'; logger.warning(f"[CORE YT Process] [{video_id}] Testo trascrizione invalido per chunking.")

                    # 5.c Embedding e Chroma
                    if current_video_status == 'processing_embedding' and chunks:
                         logger.debug(f"[CORE YT Process] [{video_id}] Embedding {len(chunks)} chunks...")
                         try:
                            ...
                            if embeddings and len(embeddings) == len(chunks):
                                logger.info(f"[CORE YT Process] [{video_id}] Embedding OK.")
                                # --- BLOCCO UPSERT CHROMA ---
                                if chroma_collection_to_use:
                                    ids = [f"{video_id}_chunk_{i}" for i in range(len(chunks))]
                                    metadatas_chroma = [{ # Costruisci metadati per Chroma
                                    ...
                                    try:
                                        logger.info(f"[CORE YT Process] [{video_id}] Upsert ChromaDB ({collection_name_for_log})...")
                                        chroma_collection_to_use.upsert(ids=ids, embeddings=embeddings, metadatas=metadatas_chroma, documents=chunks)
                                        logger.info(f"[CORE YT Process] [{video_id}] Chroma OK.")
                                        current_video_status = 'completed' # Completato SOLO se upsert OK
                                    except Exception as chroma_e:
                                        logger.exception(f"[CORE YT Process] [{video_id}] Chroma Error durante upsert.");
                                        current_video_status = 'failed_chroma_write'; chroma_errors += 1
                                else: # Collezione non disponibile
                                     logger.warning(f"[CORE YT Process] [{video_id}] Salto Upsert ChromaDB, collezione '{collection_name_for_log}' non disponibile.")
                                     current_video_status = 'failed_chroma_write'; chroma_errors += 1
                                # --- FINE BLOCCO UPSERT CHROMA ---
                            else: # Errore embedding
                                logger.error(f"[CORE YT Process] [{video_id}] Embedding fallito (None o mismatch)."); current_video_status = 'failed_embedding'; embedding_errors += 1
                         # Gestione eccezioni specifiche embedding
                         except google_exceptions.ResourceExhausted: current_video_status = 'failed_embedding_ratelimit'; embedding_errors += 1; logger.error(f"[CORE YT Process] [{video_id}] Rate limit embedding.")
                         except google_exceptions.GoogleAPIError: current_video_status = 'failed_embedding_api'; embedding_errors += 1; logger.error(f"[CORE YT Process] [{video_id}] API error embedding.")
                         except Exception as e_emb: current_video_status = 'failed_embedding'; embedding_errors += 1; logger.exception(f"[CORE YT Process] [{video_id}] Embed call error.")

                    # Se era senza chunk ma trascrizione OK -> completo
                    elif current_video_status == 'completed':
                         logger.info(f"[CORE YT Process] [{video_id}] Video completo (senza embedding/chroma per assenza di chunk).")

                except Exception as e_video_proc: # Errore generico nel ciclo del singolo video
                    logger.exception(f"[CORE YT Process] Errore processo video {video_id}.")
                    current_video_status = 'failed_processing'; generic_errors += 1

                # 5.d Prepara dati DB (sempre, anche se fallito)
                video_data_dict = video_model.dict()
                video_data_dict.update({
                    'transcript': transcript_text, 'transcript_language': transcript_lang,
                    'captions_type': transcript_type, 'processing_status': current_video_status,
                    'user_id': user_id # Aggiungi sempre user_id corretto
                })
            ...
            if processed_videos_data_for_db:
                ...
                try:
                    # Usiamo INSERT OR REPLACE per gestire casi limite (anche se non dovrebbero esserci doppioni qui)
                    # e per aggiornare lo stato se per caso un video era già 'pending'
                    ...
                    data_to_insert = [ (
                        vd.get('video_id'), vd.get('title'), vd.get('url'), vd.get('channel_id'),
                        str(vd.get('published_at')), vd.get('description'), vd.get('transcript'),
                        vd.get('transcript_language'), vd.get('captions_type'), vd.get('user_id'),
                        vd.get('processing_status', 'failed') # Default a failed se manca
                       ) for vd in processed_videos_data_for_db
                    ]
                    cursor_sqlite.executemany(sql_insert, data_to_insert)
                    conn_sqlite.commit() # Commit DOPO l'executemany
                    saved_ok_count = cursor_sqlite.rowcount # rowcount per executemany può essere -1 o il numero di righe
                    if saved_ok_count == -1: saved_ok_count = len(data_to_insert) # Stima se rowcount non è utile
                    logger.info(f"[CORE YT Process] Salvataggio batch SQLite OK ({saved_ok_count} righe affette/inserite).")
                    # Se il salvataggio ha successo, consideriamo l'operazione generale un successo
                    # anche se ci sono stati errori individuali nei video (verranno loggati)
                    overall_success = True
                except sqlite3.Error as e_sql_write:
                    logger.error(f"[CORE YT Process] ERRORE BATCH SQLITE: {e_sql_write}", exc_info=True)
                    if conn_sqlite: conn_sqlite.rollback()
                    overall_success = False # Errore critico nel salvataggio DB
                except Exception as e_prep:
                    logger.error(f"[CORE YT Process] ERRORE PREPARAZIONE BATCH SQLITE: {e_prep}", exc_info=True)
                    if conn_sqlite: conn_sqlite.rollback()
                    overall_success = False

            else: # Nessun video da salvare nel batch
                 overall_success = True # Successo se non c'era nulla da salvare

    # --- GESTIONE ECCEZIONI GLOBALI CORE ---
    except ValueError as ve: # Es. User ID mancante in SAAS
         logger.error(f"[CORE YT Process] Errore di valore: {ve}")
         overall_success = False
        ...
        logger.error(f"[CORE YT Process] Errore SQLite esterno al ciclo: {e_sql_outer}", exc_info=True)
        if conn_sqlite: conn_sqlite.rollback()
        overall_success = False
    except Exception as e_core_generic:
        logger.exception(f"[CORE YT Process] Errore generico imprevisto.")
        if conn_sqlite: conn_sqlite.rollback()
        overall_success = False
    finally:
        # --- BLOCCO FINALLY CORE ---
        if conn_sqlite:
            ...
            except Exception as close_err: logger.error(f"[CORE YT Process] Errore chiusura DB: {close_err}")
        ...
        else: logger.error(log_summary) # Logga come errore se fallito
...
def _background_channel_processing(app_context, channel_url: str, user_id: Optional[str], initial_status: dict):
    ...
    with app_context: # Esegui nel contesto app per accedere a config, etc.
        try:
            ...
            # --- 2. Chiama la Funzione Core ---
            # La funzione core gestisce la sua connessione DB e configurazione
            # e richiede il contesto app attivo (che abbiamo con `with app_context`)
            job_success = _process_youtube_channel_core(channel_id_extracted, user_id)
